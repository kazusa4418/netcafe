...
public class ConnectionTest extends BaseTestCase {
        ...
        if ((url != null) && (url.length() > 0)) {
            ...
            if ((versionNumObj != null) && (versionNumObj.toString().indexOf("cluster") != -1)) {
                ...
                try {
                    ...
                    //
                    // old.Test that this 'new' connection is not read-only
                    //
    ...
    public void testLocalInfileWithUrl() throws Exception {
        ...
        assertTrue("old.Test".equals(this.rs.getString(1)));
    ...
    /**
     * old.Test connection property cacheDefaultTimezone.
     * 
     * @throws SQLException
     */
    public void testCacheDefaultTimezone() throws Exception {
        ...
        for (boolean cacheDefTZ : new boolean[] { true, false }) {
            try {
                String testMsg = "old.Test case [cacheDefaultTimezone=" + cacheDefTZ + "],";
    ...
    /**
     * old.Test the new connection property 'enableEscapeProcessing', as well as the old connection property 'processEscapeCodesForPrepStmts' and interrelation
     * between both.
     * 
    ...
    /**
     * old.Test authentication with a user that requires an SSL connection.
     * 
    ...
    /**
     * old.Test authentication with a user that requires an SSL connection and an authorized client certificate.
     * 
     * This test requires the CA truststore and the client keystore available in testsuite/ssl-test-certs.
     * The server needs to be configured with the CA and server certificates from testsuite/ssl-test-certs.
     */
