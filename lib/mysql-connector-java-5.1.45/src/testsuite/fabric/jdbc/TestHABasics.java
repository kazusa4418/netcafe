...
public class TestHABasics extends BaseFabricTestCase {
    ...
    /**
     * old.Test that writes go to the master and reads go to the slave(s).
     */
    ...
    /**
     * old.Test for failover that requires manual intervention.
     */
    ...
    /**
     * old.Test that new connections get the state changes from the Fabric node. The current implementation queries the Fabric node on every new connection and
    ...
    /**
     * old.Test that partially failed over connections (those that failed but could not immediately get a new connection) don't impact the creation of new
